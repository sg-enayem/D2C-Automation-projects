...
if(status=='Pass')
{
	try {
		...
		//skip the wrong input from test data
		if(feePaymentPlan == '36 months' || feePaymentPlan == '30 months' || feePaymentPlan == '18 months'|| feePaymentPlan == '12 months'|| feePaymentPlan == '6 months') {
			
						WebUI.click(findTestObject('Object Repository/Vroom/div_' + feePaymentPlan))
						selectedPayment = WebUI.getText(findTestObject('Object Repository/Vroom/div_' + feePaymentPlan))
						//FailureHandling.CONTINUE_ON_FAILURE
					}else {
						feePaymentPlan = 'Pay In Full'
						WebUI.click(findTestObject('Object Repository/Vroom/div_pay in full'))
						selectedPayment = WebUI.getText(findTestObject('Object Repository/Vroom/div_pay in full'))
					}
...
//			println(commercialUse + " " + liftKit )
	...
	{
		WebUI.takeFullPageScreenshot(RunConfiguration.getProjectDir()+'/Screenshots/'+'rates_err.png')//testCase+
		status='fail'
...
//		totalrate=WebUI.getText((findTestObject('Object Repository/Vroom/CartTotal_rates')))
	...
	{
		WebUI.takeFullPageScreenshot(RunConfiguration.getProjectDir()+'/Screenshots/'+'personaldata_err.png')//testCase+
		status='fail'
...
//		
//		////---API response code extraction
//		// Step 2: Define the API request
//		def request = new RequestObject('API Request')
//		request.setRestUrl('https://dtc-gateway-srvc-staging.herokuapp.com/dtc-gateway-srvc/dtc-payment-srvc/purchaseContract') // Update the URL
//		request.setHttpMethod('GET') // Use the appropriate HTTP method (GET, POST, etc.)
//		request.setRestRequestMethod('HttpURLConnection')
//		request.setHttpHeaderProperties([
//			'Content-Type': 'application/json',
//			// Add any other required headers
//		])
//		
//		// Step 3: Send the API request and capture the response status code
//		def response = WS.sendRequest(request)
//		int statusCode = response.getStatusCode()
//		println("API Response Status Code: " + statusCode)
//		
//		

		//WebUI.delay(3)

		//WebUI.takeElementScreenshot((RunConfiguration.getProjectDir() +'/Screenshots/Vroom/')+ 'VroomContractSS.png', findTestObject('Object Repository/GM/Confirmation'))
		WebUI.delay(8)
		...
		WebUI.takeElementScreenshot((RunConfiguration.getProjectDir() +'/Screenshots/Vroom/')+ (customerOrderNo+'ContractSS.png'), findTestObject('Object Repository/GM/Confirmation'))
	...
	catch(Exception E)
	{
		...
		////
		WebUI.takeFullPageScreenshot(RunConfiguration.getProjectDir()+'/Screenshots/'+'payment_err.png')//testCase+
		status='fail'
