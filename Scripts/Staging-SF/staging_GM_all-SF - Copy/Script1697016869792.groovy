...
if (testCase != '') { 
    ...
    excelKeywords.deleteExcel(System.getProperty('user.dir'), 'Result.xlsx', 'GM_Demo_result')
}
...
def writeResult() {
    ...
    (valueToWrite[19]) = link1

    (valueToWrite[20]) = link2

    (valueToWrite[21]) = link3

    (valueToWrite[22]) = link4

    (valueToWrite[23]) = link5

    (valueToWrite[24]) = link6

    (valueToWrite[25]) = link7

    (valueToWrite[26]) = link8
	
	(valueToWrite[27]) = link8
...
//    (valueToWrite[35]) = link8 
   ...
   // excelKeywords.writeExcel(System.getProperty('user.dir'), 'GM_Results.xlsx', 'GM_Portals_Result', valueToWrite)
	// Create a new ExcelKeywords instance for working with Excel files
    ExcelKeywords excelKeywords = new ExcelKeywords()

    // Define the file path and sheet name
    String filePath = System.getProperty('user.dir') + '/Result.xlsx'
    String sheetName = 'GM_Demo_result'

    // Append the data to the Excel file
    excelKeywords.appendRowToExcel(filePath, sheetName, valueToWrite)
}
