...
//		request.setRestUrl('https://d2c-gateway-staging.herokuapp.com/dtc-gateway-srvc/dtc-payment-srvc/purchaseContract') // Update the URL
//		request.setHttpMethod('POST') // Use the appropriate HTTP method (GET, POST, etc.)
...
//		
		...
		if(WebUI.getUrl()==baseURL+'/checkout/confirmation' && WebUI.getUrl()==baseURL+'checkout/confirmation' )
			{
				WebUI.takeFullPageScreenshot(RunConfiguration.getProjectDir()+'/Screenshots/Vroom'+testCase+'VroomContract.png')
				status='Pass'
