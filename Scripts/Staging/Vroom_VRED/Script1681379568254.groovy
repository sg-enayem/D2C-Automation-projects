				...
				if ((verifyDB == 'Success') || (verifyDB != 'Success')) {
					...
					while (rs.next()) {
						if ((rs.getString('order_summary_id') == orderSummaryId) && (rs.getString('data') != null)) {
							...
							CustomKeywords.'pdf_verification.db_binary_pdfCheck_vroom_VRED.Robot_key_press'()

							CustomKeywords.'pdf_verification.db_binary_pdfCheck_vroom_VRED.download_pdf_from_binary'()
								

					Map<String, String> results = CustomKeywords.'pdf_verification.db_binary_pdfCheck_vroom_VRED.download_pdf_from_binary'()
...
//						String lastName = "Doe"
				...
				def pdfResult = ""
					pdfResult = CustomKeywords.'pdf_verification.db_binary_pdfCheck_vroom_VRED.verifyPdf'(results, deductible, termLength, subtotal, firstName, lastName)
