...
//use for javascriptExecutor for 'continue to billing ' button->
...
if (status == 'Pass') {
    try {
       ...
       // WebUI.click(findTestObject('Object Repository/Vroom/button_Continue to Billing - Copy'))//change note-xpath
		WebUI.delay(3)
...
if (status == 'Pass') {
    try {
		...
		WebUI.waitForElementPresent(findTestObject('Object Repository/GM_Brands/p_orderid'), 60)
//		
...
// Write code here that turns the phrase above into concrete actions
if (status == 'Pass') {
	WebUI.verifyElementPresent(findTestObject('Object Repository/GM_Brands/p_orderid'), 5)

	customerOrderNo = WebUI.getText(findTestObject('Object Repository/GM_Brands/p_orderid')).substring(16)
