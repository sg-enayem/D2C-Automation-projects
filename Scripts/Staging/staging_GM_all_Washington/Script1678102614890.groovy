...
if (status == 'Pass') {
    try {
        ...
        if (deductible == '100') {
            WebUI.waitForElementClickable(findTestObject('Object Repository/GM/Page_GMC - Get Rates/WA_100_deductable'), 30)

            UtilKeywords utilKeywordst = new UtilKeywords()

            utilKeywordst.clickUsingJS(findTestObject('Object Repository/GM/Page_GMC - Get Rates/WA_100_deductable'), 30)
        }
        
        if (deductible == '250') {
            WebUI.waitForElementClickable(findTestObject('Object Repository/GM/Page_GMC - Get Rates/WA_250_deductable'), 30)

            UtilKeywords utilKeywordst = new UtilKeywords()

            utilKeywordst.clickUsingJS(findTestObject('Object Repository/GM/Page_GMC - Get Rates/WA_250_deductable'), 30 //CustomKeywords.'com.example.WebUIKeywords.clickUsingJS'(findTestObject('Object Repository/GM/Page_GMC - Get Rates/deductible_250'), 30)
        ...
        //new code/
        //			if(WebUI.verifyElementNotVisible(findTestObject('Object Repository/GM/Page_GMC - Get Rates/span_lever1'), FailureHandling.CONTINUE_ON_FAILURE) == true){
        //				System.out.println(" commercialUse element is visible " );
        //				 }
        //System.out.println('okokokokokokokokokokokokokokokokokokok')
...
CustomKeywords.'pdf_verification.pdf_url_check.first_link_GMC'()
	...
	link2 = "Not applicable for WA"

//verify pdf is downloadable or not : link_02



	link3 = "Not applicable for WA"
...
if (status == 'Pass') {
    try {
        ...
        est_total = WebUI.getText(findTestObject('Object Repository/Vroom/Total_rate'))
       ...
       // println((subtotal + '     ') + salesTax)
        ...
        Trate = subtotal.replaceAll('\\$|\\.\\d+|,', '' // new code
            )
    ...
    try {
        ...
        /////
			...
			link5 = 'PDF working:Cart Page_Billingt Page->VIEW AGREEMENT AND EXCLUSIONS'
						...
						link7 = 'Not applicable for Washington State'
		...
		while (rs.next()) {
			...
			if (feePaymentPlan != 'Pay In Full') {
				...
				/* &&
					...
					//M
					first_query_result = 'Status is Active || Customer Order number is matched' //println('###---Status is Active || Customer Order number is matched---###')
...
//						String lastName = "Doe"
			...
			//		PGDatabaseConnection pd = new PGDatabaseConnection()
			//		String str5 = ('select * from public.contract_pdf where order_summary_id=\'' + orderSummaryId) + '\''
			//
			//		rs = pd.dataBaseConnection(str5, GlobalVariable.pghost, GlobalVariable.pgdbInstance, GlobalVariable.pgUser,
			//				GlobalVariable.pgPassword)
