...
if(status=='Pass') {
	try {
		...
		totalDueToday_checkout = WebUI.getText((findTestObject('Object Repository/Vroom/nissan_checkout_TotalDueToday')))
		trimmed_totalrate = totalrate.replaceAll(/[^0-9.]/, '') as float
				...
				if ((verifyDB == 'Success') || (verifyDB != 'Success')) {
					...
					while (rs.next()) {
						if ((rs.getString('order_summary_id') == orderSummaryId) && (rs.getString('data') != null)) {
							...
							CustomKeywords.'pdf_verification.db_binary_pdfCheck_Nissan.Robot_key_press'()

							CustomKeywords.'pdf_verification.db_binary_pdfCheck_Nissan.download_pdf_from_binary'()
								

					Map<String, String> results = CustomKeywords.'pdf_verification.db_binary_pdfCheck_Nissan.download_pdf_from_binary'()
...
//						String lastName = "Doe"
				...
				def pdfResult = ""
					pdfResult = CustomKeywords.'pdf_verification.db_binary_pdfCheck_Nissan.verifyPdf'(results, deductible, termLength, subtotal, firstName, lastName)
...
//Runtime.getRuntime().exec(cmd1)
			...
			println(str6)
			ResultSet rs1=te.dataBaseConnection(str6, 'qtsuatpost6:54262','forte', 'enayem', 'mun@Abir##22')
			//String orderId=''
			//println(res)s
			while ( rs1.next() ) {
			...
			ResultSet rs2=te.dataBaseConnection(str7, 'qtsuatpost6:54262','forte', 'enayem', 'mun@Abir##22')
			...
			if(verifyDB=='Success' && portal !='Nissan')
				...
				ResultSet rs3=te.dataBaseConnection(str8, 'qtsuatpost6:54262','forte', 'enayem', 'mun@Abir##22')
				//String orderId=''
				println(str8)
				while ( rs3.next() ) {
					//orderId=rs1.getString('order_id__c')
