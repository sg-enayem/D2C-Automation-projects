...
if ((portal == 'GMC') || (portal == 'gmc')) {
    if (oid != '') {
            WebUI.navigateToUrl(('https://mango-rewrite-'+ portal.toString().toLowerCase()) +'-qa.herokuapp.com/vehicle-service-protection?oid=' + oid)
			baseURL = 'https://mango-rewrite-'+ portal.toString().toLowerCase()+'-qa.herokuapp.com/'
    } else {
        WebUI.navigateToUrl('https://mango-rewrite-' + portal.toString().toLowerCase() + '-qa.herokuapp.com/')

        baseURL = (('https://mango-rewrite-' + portal.toString().toLowerCase()) + '-qa.herokuapp.com/')
    }
} else if ((portal == 'Chevy') || (portal == 'chevy')) {
    if (oid != '') {
            WebUI.navigateToUrl(('https://mango-rewrite-'+ portal.toString().toLowerCase()) +'-qa.herokuapp.com/vehicle-service-protection?oid=' + oid)
			baseURL = 'https://mango-rewrite-'+ portal.toString().toLowerCase()+'-qa.herokuapp.com/'
    } else {
...
if (status == 'Pass') {
    try {
		...
		if ((portal.equalsIgnoreCase('GMC') || portal.equalsIgnoreCase('Chevrolet'))) {
			...
			println(subtotalWithSurcharge +" " + subtotal)
...
if (status == 'Pass') {
    try {
		...
		if (feePaymentPlan != 'Pay In Full') {
			...
			subtotal=WebUI.getText(findTestObject('Object Repository/GM_Brands/rates_subtotal'))
			salesTax=WebUI.getText(findTestObject('Object Repository/GM_Brands/sales_tax'))
			totalrate=WebUI.getText((findTestObject('Object Repository/GM_Brands/total_rates')))
			
		strrr = WebUI.getText((findTestObject('Object Repository/GM_Brands/checkOutCalculation')))
		totalDueToday_checkout = WebUI.getText((findTestObject('Object Repository/GM_Brands/checkout_TotalDueToday')))
...
// account password, as DBs password
        ResultSet rs1 = te.dataBaseConnection(str6, 'qtsuatpost6:54262', 'forte', 'enayem', 'EN@12345#')
        ...
        if (verifyDB == 'web_con_m1 Success') {
            ...
            ResultSet rs2 = te.dataBaseConnection(str7, 'qtsuatpost6:54262', 'forte', 'enayem', 'EN@12345#')
        ...
        if (verifyDB == 'dms_con_m1 Success') {
            ...
            ResultSet rs3 = te.dataBaseConnection(str8, 'qtsuatpost6:54262', 'forte', 'enayem', 'EN@12345#')
