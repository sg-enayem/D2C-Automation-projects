...
if(portal.equalsIgnoreCase('nissan') && portal_type.equalsIgnoreCase('asp')) 
  {
	  println(portal_type)
	  if(oid !='') {WebUI.navigateToUrl(GlobalVariable.NisssanASP_URL+oid)}
	  else {WebUI.navigateToUrl('https://www.mynissanprotection.ca/added-security-plan?oid=' + oid)}
	  
	  baseURL='https://mynissanprotection.ca/'
  ...
  {
	  println(portal_type)
	  if(oid !='') {WebUI.navigateToUrl(GlobalVariable.NissanPPM_URL+oid)}
	  else {WebUI.navigateToUrl('https://www.mynissanprotection.ca/prepaid-maintenance?oid=' + oid)}
...
if(WebUI.getUrl()=='https://www.mynissanprotection.ca/my-asp-pricing?plan=N1' || WebUI.getUrl()=='https://www.mynissanprotection.ca/my-pmp-pricing')
...
//////////////////////////
	...
	WebUI.delay(5)
	if(WebUI.getUrl()!='https://www.mynissanprotection.ca/cart')
	{
...
//			}
	...
	WebUI.delay(5)
	if(WebUI.getUrl()!='https://www.mynissanprotection.ca/checkout')
	{
		WebUI.takeFullPageScreenshot(RunConfiguration.getProjectDir()+'/Screenshots/'+testCase+'rates_err.png')
		status='fail'
		error_msg='Failed to move to checkout page'
...
//			(valueToWrite[26]) = eightth_query_result
		...
		ExcelKeywords excelKeywords = new ExcelKeywords()
		excelKeywords.writeExcel(System.getProperty('user.dir'), 'Result - Prod.xlsx', 'Nissan_Result', valueToWrite)
